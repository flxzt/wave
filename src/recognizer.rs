//! the main entry point in the library.
//!
...
pub enum Gesture {
    ...
    GestureNone = 0,
    /// A static hold, meaning the hand is hovering still for a period of time
    GestureStaticHold,
    /// A right swipe
    GestureSwipeRight,
    /// A left swipe
...
pub struct RecognizerResult {
    ...
    /// The recognized gesture, GestureNone if no gesture was found.
    pub gesture: Gesture,
}
...
pub struct RecognizerParams {
    ...
    /// How much the hand can move from and towards the sensor while doing a static hold
...
pub enum RecognizerStatus {
    ...
    /// Indicates a failure to initialize the recognizer
...
/// The history size.
///
/// For users of the C bindings: do not modify!
/// This is not meant to be part of the public API, but needs to be exposed to make the bindings work.  
/// Is at least ca. large enough to hold ca 2secs of data to be able to reliably recognize gestures.
pub const HISTORY_SIZE: usize = 60;

/// The gesture recognizer. Is initially configured through parameters and gets fed measurements and time and predicts gestures based on its input.
#[repr(C)]
#[derive(Debug, Clone)]
pub struct GestureRecognizer<const RES_X: usize, const RES_Y: usize> {
...
impl<const RES_X: usize, const RES_Y: usize> GestureRecognizer<RES_X, RES_Y> {
    ...
    ///
    /// Returns either GestureNone if no swipe is found or GestureSwipeRight / GestureSwipeLeft
